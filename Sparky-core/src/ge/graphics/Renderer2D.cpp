...
#define RENDERER_MAX_TEXTURES	32 - 1
	...
	Renderer2D::Renderer2D(uint width, uint height)
		: m_IndexCount(0), m_ScreenSize(tvec2<uint>(width, height)), m_ViewportSize(tvec2<uint>(width, height)), m_Mask(nullptr), m_PostEffectsEnabled(false)
	{
		Init();
	}

	Renderer2D::Renderer2D(const tvec2<uint>& screenSize)
		: m_IndexCount(0), m_ScreenSize(screenSize), m_ViewportSize(screenSize), m_Mask(nullptr), m_PostEffectsEnabled(false)
	{
		Init();
	}
	...
	float Renderer2D::SubmitTexture(API::Texture* texture)
	{
#if 0
		if (!textureID)
	...
	void Renderer2D::DrawLine(const maths::vec2& start, const maths::vec2& end, uint color, float thickness)
	{
		DrawLine(start.x, start.y, end.x, end.y, color, thickness);
	}

	void Renderer2D::DrawRect(float x, float y, float width, float height, uint color)
	{
	...
	void Renderer2D::DrawRect(const maths::vec2& position, const maths::vec2& size, uint color)
	{
		DrawRect(position.x, position.y, size.x, size.y, color);
	}

	void Renderer2D::DrawRect(const Rectangle& rectangle, uint color)
	{
		DrawRect(rectangle.GetMinimumBound(), rectangle.size * 2.0f, color);
	}

	void Renderer2D::DrawString(const String& text, const maths::vec2& position, const Font& font, uint color)
	{
	...
	void Renderer2D::FillRect(const maths::vec2& position, const maths::vec2& size, uint color)
	{
		FillRect(position.x, position.y, size.x, size.y, color);
	}

	void Renderer2D::FillRect(const Rectangle& rectangle, uint color)
	{
